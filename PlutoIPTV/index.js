...
function processChannels(version, list) {
  ...
  channels.forEach((channel) => {
    ...
    if (
      ...
      m3u8 =
        m3u8 +
        `#EXTINF:0 channel-id="${channel.number}" channel-number="${channel.number}" tvg-id="${channel.number}" tvg-chno="${channel.number}" tvg-logo="${logo}" tvc-guide-art="${art}" tvc-guide-title="${name}" tvc-guide-description="${guideDescription}" group-title="${group}", ${name}
${m3uUrl}
